...
@Service
public class EmployerSignupCodeCryptoService {
    ...
    public EmployerSignupCodeCryptoService(
            @Value("${employer.signup-code-encryption-key}") String encodedKey
    ) {
    ...
    public String encrypt(String plainText) {
        try {
            ...
            throw new IllegalStateException("employer signup code encryption failed", e);
        }
    }
    ...
    public String decrypt(String encryptedText) {
        try {
            ...
            throw new IllegalStateException("employer signup code decryption failed", e);
        }
    }
    ...
    private EncryptedPayload decodePayload(String encryptedText) {
        ...
        try {
            ...
            throw new IllegalArgumentException("employer signup code payload is not valid base64", e);
        }

        if (payload.length <= GCM_IV_BYTES) {
            throw new IllegalArgumentException("employer signup code payload is too short");
        }
    ...
    private SecretKeySpec decodeSecretKey(String encodedKey) {
        if (encodedKey == null || encodedKey.isBlank()) {
            throw new IllegalStateException("EMPLOYER_SIGNUP_CODE_ENCRYPTION_KEY is required");
        }
        ...
        try {
            ...
            throw new IllegalStateException("EMPLOYER_SIGNUP_CODE_ENCRYPTION_KEY must be valid base64", e);
        }

        if (!isSupportedAesKeyLength(keyBytes.length)) {
            throw new IllegalStateException("EMPLOYER_SIGNUP_CODE_ENCRYPTION_KEY must decode to 16, 24, or 32 bytes");
        }
