...
- 기록
  ...
  - `Slice 2 -> feat: 근무 기록 인정 시간 조회 모델 추가` (`efff435`)
  - `Slice 3 -> feat: 검토 요청 규칙과 자동 보정 분기 추가` (`15e72da`)
  - `Slice 4 -> feat: 고용주 검토 승인/반려 흐름 정렬` (`f927e5f`)
  - `Slice 5 -> feat: 모바일 근무 수정 요청 연동 추가` (`9a147c2`)
  - `Slice 6 -> feat: 웹 설정과 검토 화면 정책 계약 연동` (`9cecc56`)
  - `Slice 7 -> fix: 인정 시간 기준 wage와 문서 계산 정렬` (`ea9e4fc`)
  - `Slice 7 -> test: advance 인정 시간 회귀 검증 추가` (`f5aa203`)
...
- 다음 구현 시작점은 Slice 6이며, Slice 6 전에는 wage/advance 후속 정리를 먼저 열지 않는다.
