...
## Affected Modules
### Backend
- `remittance/api/dto/response/TransferListItemResponse.java`
- `remittance/api/dto/response/TransferDetailResponse.java`
- `remittance/repo/TransferRepository.java`
- `remittance/service/TransferService.java`
...
## Contract Changes
- remittance transfer 조회 응답에 `direction` 추가
- remittance transfer summary 응답에 `senderAddress` 추가
- remittance transfer 조회 응답에 `networkFeeWei`, `networkFeeAssetSymbol` 추가
- `/api/remittance/transfers` 와 `/api/remittance/transfers/{transferId}` 가 발신/수신 모두 조회 가능하도록 확장
- 모바일 내부 UI 계약 추가:
